...
/**
 * Observacao do relatorio: algo fora do comum na coleta (FR-001).
 *
 * <p>Opcional. A vazia, ou so de espacos, vira ausente, como a descricao do setor.
 *
 * @param value observacao ja aparada, de 1 a 500 caracteres
 */
public record ReportNote(String value) {
    ...
    private static final List<Rule<String>> RULES = List.of(Rule.of(
            ...
            ProductionErrorCode.REPORT_NOTE_TOO_LONG,
     ...
     */
    public static Optional<ReportNote> optionalOf(String raw) {
        Notification notification = new Notification();
        validate(raw, notification);
        notification.throwIfAny(ProductionErrorCode.VALIDATION_FAILED);
        return raw == null || raw.isBlank() ? Optional.empty() : Optional.of(new ReportNote(raw.strip()));
    }
